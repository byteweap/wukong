...
import (
	...
	nacosdiscovery "github.com/byteweap/wukong/contrib/registry/nacos"
)
...
func runProvider() {
	...
	log.Printf("✅ 服务已注册: %s (ID: %s, Port: %d, Namespace: %s, Group: %s)",
		*serviceName, instanceID, *port, *namespace, *group)
...
func runConsumer() {
	// 创建服务发现器
	discovery, err := nacosdiscovery.NewDiscovery(
		nacosdiscovery.ServerAddrs(*nacosAddr),
		nacosdiscovery.Namespace(*namespace),
		nacosdiscovery.Group(*group),
	)
	...
	// 获取服务列表
	log.Printf("🔍 查找服务: %s (Namespace: %s, Group: %s)", *serviceName, *namespace, *group)
